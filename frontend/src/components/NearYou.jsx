...
const NearYou = ({ businessdata }) => {
    const Photo = businessdata.map((business) => (
      <figure key={business.name}>
        <GridItem className='photoOfOrder' w='100%' h='10' bg='blue.500' src={business.img} wrapped ui={false} />
        <Text fontSize='xl'>{business.name}</Text>
      </figure>
    ));
